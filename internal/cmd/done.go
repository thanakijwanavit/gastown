...
func runDone(cmd *cobra.Command, args []string) error {
	...
	// cwd is the town root, not the polecat's worktree. Detect this and
	// reconstruct the actual polecat clone path for correct git operations.
	if cwdAvailable && cwd == townRoot {
		if polecatName := os.Getenv("GT_POLECAT"); polecatName != "" && rigName != "" {
			// Try new worktree layout: <rig>/polecats/<polecat>/<rig>/
			polecatClone := filepath.Join(townRoot, rigName, "polecats", polecatName, rigName)
			if _, err := os.Stat(polecatClone); err == nil {
				cwd = polecatClone
			} else {
				// Try old layout: <rig>/polecats/<polecat>/
			...
			// Crew worker at town root - reconstruct crew clone path
